<?php
    ...
    if(isset($_GET['post_id'])){
        ...
        $delete = mysqli_query($conn, "DELETE FROM publication WHERE id = '$post_id'") or die("query failed");

        if($delete){
            $_SESSION['id'] = $id;
            //Check if there's comment in this post

            $Select_cm = mysqli_query($conn, "SELECT * FROM comments WHERE post_id = '$post_id'");

            if($Select_cm){

                //Delete all comments in this post

                $delete_cm = mysqli_query($conn, "DELETE FROM comments WHERE post_id = '$post_id'");
